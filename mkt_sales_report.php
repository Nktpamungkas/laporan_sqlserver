                                                    ...
                                                    <tbody>
                                                        <?php 
                                                            ...
                                                            $q_salesreport = "SELECT *, 
                                                                CASE 
                                                                    WHEN LEN(tax_ID) = 15 THEN 
                                                                        LEFT(tax_ID, 2) + '.' + SUBSTRING(tax_ID, 3, 3) + '.' + SUBSTRING(tax_ID, 6, 3) + '.' + SUBSTRING(tax_ID, 9, 1) + '-' + SUBSTRING(tax_ID, 10, 3) + '.' + SUBSTRING(tax_ID, 13, 3)
                                                                    ELSE 
                                                                        '0' + LEFT(tax_ID, 1) + '.' + SUBSTRING(tax_ID, 2, 3) + '.' + SUBSTRING(tax_ID, 5, 3) + '.' + SUBSTRING(tax_ID, 8, 1) + '-' + SUBSTRING(tax_ID, 9, 3) + '.' + SUBSTRING(tax_ID, 12, 3)
                                                                END AS NPWP,
                                                            ...
                                                            FROM 
                                                                invoice.[laporan bulanan (payment)]
                                                            WHERE datenow BETWEEN ? AND ?";

                                                            $params = array($_tgl1, $_tgl2);
                                                            $stmt = sqlsrv_query($con_invoice, $q_salesreport, $params);
