    ...
    <head>
        ...
        <script>
            function fetchDataAndUpdateTable() {
                ...
                xhr.open('GET', 'r_fetch_data.php', true);
            ...
            function fetchDataAndInsertQuery(data) {
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    xhrInsert.open('POST', 'r_insert_data.php', true);
            ...
            function updateTable(data) {
                ...
                var table = `<table class="table table-striped table-bordered nowrap" width="100%">
                                <thead>
                                    <th>SCHEMA NAME</th>
                                    <th>QUERY</th>
                                    <th>JML_TOTAL_EXC_QUERY</th>
                                    <th>TIME_IN_MINUTES</th>
                                    <th>TIME_IN_HOUR</th>
                                    <th>LAST_SEEN</th>
                                </thead>`;

                // Menambahkan baris untuk setiap elemen data
                for (var i = 0; i < data.length; i++) {
                    table += `<tbody>
                                <tr>
                                    <td>`+ data[i].SCHEMA_NAME +`</td>
                                    <td>`+ data[i].QUERY +`</td>
                                    <td>`+ data[i].JML_TOTAL_EXC_QUERY +`</td>
                                    <td>`+ data[i].TIME_IN_MINUTES +`</td>
                                    <td>`+ data[i].TIME_IN_HOUR +`</td>
                                    <td>`+ data[i].LAST_SEEN +`</td>
                                </tr>
                            </tbody>`;
                }
